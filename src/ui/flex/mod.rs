...
use crate::{
    ecs::{
        Changed, Entity, EventReader, Query, ReadOnlyWorldQuery, RemovedComponents, Res, ResMut,
        Resource, With, Without,
    },
    transform::{Children, Parent, Transform},
    ty::Vec2,
    windowing::{Window, WindowId, WindowScaleFactorChanged, Windows},
};

use super::{CalculatedSize, Node, Style, UiScale};
...
pub fn flex_node_system(
    ...
    for (entity, mut node, mut transform, parent) in &mut node_transform_query {
        ...
        if let Some(parent) = parent {
            if let Ok(parent_layout) = flex_surface.get_layout(parent.get()) {
